...
close all;
%%
t=csvread("sinwave4.csv");   % sin4 is giving best results
time=t(:,1);
y=t(:,2);
...
sigphase3=unwrap(sigphase);
sigphase4=sigphase3+k(5);
sigphase5=wrapToPi(sigphase4);

subplot(5,1,4); grid on; box on;
plot(sigphase5)
title("Real Time Phase")
%%
squareWave=zeros(length(sigphase5),1);
for j=1:length(sigphase5)
    if (1<= sigphase5(j)) && (sigphase5(j)<=1.5)
        squareWave(j,1)=1;
    end
end
subplot(5,1,5)
...
%writematrix([time squareWave squareWave1],'squareWaves_After_phaseCompensation.csv')
%%
